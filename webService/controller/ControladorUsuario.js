	...
	return {
		registro:function(peticion, respuesta){
            modelo.sequelize.query( "CALL sp_registroUsuario ('"+peticion.body.nombre+"', '"+peticion.body.telefono+"', '"+peticion.body.correo+"', '"+peticion.body.nick+"', '"+peticion.body.password+"')")
                .then(function(){
					respuesta.send({ "mensaje":"Registrado exitosamente","status":"200"});
				}).error(function(err){
				    respuesta.send({"mensaje":"Error. No se agrego "+err,"status":"500"});
			});
		},
		login:function(peticion, respuesta){
            modelo.sequelize.query("CALL sp_autenticarUsuario ('"+peticion.body.correo+"', '"+peticion.body.contrasena+"')")
                .then(function(user){
					if(user.length>0)
						respuesta.json(getToken(user));
					else
						respuesta.json({"user":[]});
			}).error(function(err){
